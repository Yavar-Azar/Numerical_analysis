...
real  a,b,d,delta,diff,sum1,sum2,sumdiff,area
...
sumdiff=0
diff=0
allocate(charge(nrow,3))


open(unit=1, file=data1, status='old', iostat=stat)

	do i=1,nrow
		read(1,*)  charge(i,1), charge(i,2), charge(i,3)
	end do
...
open(unit=2, file=newdata, status='new')
	...
	do j=1,nrow
	diff=(charge(j,3)-charge(j,2))
	sum1=charge(j,2)*delta+sum1
	sum2=charge(j,3)*delta+sum2
	sumdiff=diff*delta+sumdiff
      write(2,*) charge(j,1)*0.52917721, a*charge(j,2)/b, a*charge(j,3)/b, a*diff/b, a*sum1/d, a*sum2/d, a*sumdiff/d
	end do
end program
