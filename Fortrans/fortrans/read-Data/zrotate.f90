!this program reads structure file from xyz format input and rotate about z  

program zrotatestruct
implicit none
...
open(unit=2, file=rotated, status='new')
	...
	write(2,*)
	do j=1,nat
		write(2,*) element(j), a*posit(j,1)-b*posit(j,2),b*posit(j,1)+a*posit(j,2),posit(j,3)
	end do
end program
