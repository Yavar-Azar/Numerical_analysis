...
!reading struct file
write(*,*)  '***********************************CONVOLUTION***********************************************************'
write(*,*)  'Yavar  T. AZAR  20 March 2013 , Theoretical and Computational Physics group, AEOI'
write(*,*)  '// This program convolute oscillator strength to give extiction coefficient'
write(*,*)  'output format is colm(1)=wavelength in eV, colm(2)=wavelength in nm, colm(3)=extinction in M^-1.cm^-1'
	...
	do i=1,nstate
		read(1,*) data1(i,1), data1(i,2)
			do j=1,ngrid
				...
				y=data1(i,1)
				convmat(j,1)=x
				convmat(j,2)=convmat(j,2)+const*((data1(i,2)/fw)*exp(-2.773*(((x-data1(i,1))**2)/(fw**2))))
			end do
	end do
